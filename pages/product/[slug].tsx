...
const ProductPage: NextPage<Props> = ({ product }) => {
  ...
  const onUpdateQuantity = (newValue: number) => {
    setTempCartProduct((prev) => ({ ...prev, quantity: newValue }));
  };
          ...
          <Box display="flex" flexDirection="column">
            ...
            <Box sx={{ my: 2 }}>
              ...
              <SizeSelector
                ...
                onSeletedSize={onSeletedSize}
              />
            </Box>
            {/* Agregar al carrito */}
            {product.inStock > 0 ? (
              <Button color="secondary" className="circular-btn" sx={{ py: 1 }} onClick={onAddProduct}>
                {tempCartProduct.size ? "Agregar al carrito" : "Seleccione una talla"}
              </Button>
            ) : (
              <Chip label="No hay disponible" color="error" variant="outlined" sx={{ py: 1 }} />
            )}
