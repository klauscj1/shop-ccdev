...
const getProducts = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  await db.connect();
  const products = await Product.find().sort({ ttile: "asc" }).lean();
  await db.disconnect();
...
const createProduct = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  //TODO: posiblemente tendremos un localhost:3000/products/asasda.jpg
  try {
    await db.connect();
...
const updateProduct = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  try {
    ...
    if (!product) {
      await db.disconnect();
      return res.status(404).json({ message: "No existe un producto con este id" });
    }
